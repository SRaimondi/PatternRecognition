...
public class CSVReader {
    ...
    public static svm_node[][] setupSVMNodesArray(String file_name) {
     ...
     /**
     * Returns a list of the actual instance classes stored in the specified file
     *
     * @param file_name Name of the file to extract the values from
     * @return A list of the values
     */
    
    
    public static ArrayList<Integer> getActualValues(String file_name) {
        List<ArrayList<Integer>> file = new ArrayList<>();
        try {
            readCSVFile(file_name, file);
        } catch (Exception e) {
        ...
        ArrayList<Integer> actual_values = new ArrayList();

        for (ArrayList<Integer> x : file) {
            actual_values.add(x.get(0));
        }
